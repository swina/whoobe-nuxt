<template>
    <div>
        <div ref="sideNav" :class="'bm-menu ' + $attrs.el.css.responsive">
            <nav class="bm-item-list ml-4">
                <slot></slot>
            </nav>
            <i v-if="$route.path!='/'" class="bi-arrow-left absolute top-0 left-0 m-2" @click="$router.push('/')"></i>
            <i class="bi-x absolute top-0 right-0 text-3xl m-2" @click="closeMenu"></i>
            
        </div>
 
        <div ref="bmBurgerButton" class="bm-burger-button top-0 left-0 m-4 w-8 h-6 " @click="openMenu" :class="{ hidden: !burgerIcon }">
      ...
      watch: {
        ...
        right: {
          ...
          handler(oldValue, newValue) {
            if (oldValue) {
              this.$nextTick(() => {
                this.$refs.bmBurgerButton.classList = 'mr-6 ';
                this.$refs.sideNav.style.left = 'mr-0';
                document.querySelector('.bm-burger-button').classList = 'mr-6'
                document.querySelector('.bm-menu').classList = 'mr-0';
                /*
